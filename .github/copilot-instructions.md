...
All commits and Pull Request titles opened by Copilot must follow the Conventional Commits / semantic commit message rules. This helps keep the changelog clear and enables automated release tooling.
...
- Optionally include a scope in parentheses, e.g. `feat(sutom): add new game command`
- Keep the description short and in the imperative, present tense (e.g. "add", "fix", "update").

Examples:

- `feat(sutom): add start subcommand to create a new game`
- `fix(russian-roulette): prevent crash when no players present`
- `chore(release): publish v1.2.0`

Please ensure PR titles generated by Copilot follow the same format so automation (semantic-release, changelogs) remains consistent.

Right before finishing your response you MUST run the project's verification command to ensure formatting, linting and TypeScript checks pass.

- Always run: `npm run check`
- This command runs formatting, ESLint and TypeScript checks for the repository. You must execute it EVERY TIME before you finalize your reply to verify your work.

If `npm run check` reports issues, fix them and re-run the command until it succeeds. This ensures generated or edited code follows the project's standards and avoids trivial follow-up fixes.
